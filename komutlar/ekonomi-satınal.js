...
exports.run = async(client, message, args) => {
    if(!args[0]) return message.channel.send(`Lütfen bir eşya ismi belirtin! \n \n Eşya isimlerini bilmiyorsanız z!market komutunu kullanın`)
...
if(args[0] == "olta") {
    ...
    .setDescription(`Şu anki paran: ${enginar}  :coin: \n \n Ürünün fiyatı: 500  :coin:`)
    if(para < 500) return message.channel.send(embed)
    if(para > 500) {
        db.math(`para_${message.author.id}`,"-", 500)
        db.add(`olta_${message.author.id}`, 1)
        const embed = new discord.MessageEmbed()
        .setTitle('Satın alım başarılı!')
        .setDescription(`${message.author.username} adlı kişi 500  :coin:  vererek olta satın aldı!`)
