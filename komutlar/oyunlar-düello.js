...
exports.run = async (client, message, args) => {
  ...
  if (opponent.id === message.author.id) return message.reply('Kendin ile düello Atamazsın');
		...
		try {
			...
			while (userHP > 0 && oppoHP > 0) { // eslint-disable-line no-unmodified-loop-condition
				...
				if (!opponent.bot || (opponent.bot && userTurn)) {
					await message.channel.send(stripIndents`
						...
						**${message.author.username}**: ${userHP} \<:enkalp:813341524816691261>
						**${opponent.username}**: ${oppoHP} \<:enkalp:813341524816691261>
					`);
					const filter = res =>
			...
			return message.channel.send(`Oyun bitti! Tebrikler, **${winner}** kazandı! \<a:enmavikalp:813339262031233034> \n**${message.author.username}**: ${userHP} \<:enkalp:813341524816691261> \n**${opponent.username}**: ${oppoHP} \<:enkalp:813341524816691261>`);
		} catch (err) {
			this.fighting.delete(message.channel.id);
			throw err;
		}
