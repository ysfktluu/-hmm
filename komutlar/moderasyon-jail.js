...
if(!data)  return message.channel.send(`\<a:enhayir:813350216911683584> Jail rolünü bulamadım.`)
let data2 = await db.fetch(`jailyetkilisi_${message.guild.id}`)
if(!data2)  return message.channel.send(`\<a:enhayir:813350216911683584> Jail yetkilisi rolünü bulamadım.`)
let data3 = await db.fetch(`jailkanal_${message.guild.id}`)
if(!data3)  return message.channel.send(`\<a:enhayir:813350216911683584> Jail kanalını bulamadım.`)
let rol = message.guild.roles.cache.get(data)
if(!rol) return message.channel.send(`\<a:enhayir:813350216911683584> Jail rolü ayarlı değil.`)
let yetkilirol = message.guild.roles.cache.get(data2)
if(!yetkilirol) return message.channel.send(`\<a:enhayir:813350216911683584> Jail yetkilisi ayarlı değil.`)
let kanal = message.guild.channels.cache.get(data3)
if(!kanal) return message.channel.send(`\<a:enhayir:813350216911683584> Jail log kanalı ayarlı değil.`)
if(!yetkilirol) return message.channel.send(`Yetkili rolü ayarlanmamış!`)
  ...
  if(!kişi) return message.channel.send(`\<a:enhayir:813350216911683584>  Birini etiketle`)
  if(kişi.hasPermission("MANAGE_GUILD")) return message.channel.send(`\<a:enhayir:813350216911683584>  Seçtiğiniz kişide **MESAJLARI YÖNET** yetkisi var`)
  
  let zaman = args[1]
  if(!args[1]) return message.channel.send(`<a:no1:789904606959239208> Ne kadar süre jailde duracağını belirtmelisin.\nÖrnek: z!jail @kişi 8m çok konuştu\n\n Saat birimleri: 1d = 1 gün \n 1s = 1 saniye \n 1m = 1 dakika \n 1h = 1 saat`)
...
if(!sebep) sebep = 'Sebep belirtilmemiş.'
  ...
  kişi.roles.add(rol.id);
    ...
    kanal.send(wasted)
    message.channel.send(`\<a:enevet:813350232879923200> ${kişi} isimli kişi başarıyla tutuklandı`)
    setTimeout(async () =>{
